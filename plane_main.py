...
class PlaneGame(object):
    ...
    def __init__(self):
        ...
        # 4. 设置定时器事件 - 创建敌机　1s
        pygame.time.set_timer(CREATE_ENEMY_EVENT, random.randint(1000, 2000))  # 创建敌机
        pygame.time.set_timer(HERO_FIRE_EVENT, 400)  # 英雄发射子弹
        pygame.time.set_timer(BUFF1_SHOW_UP, random.randint(10000, 20000))  # 创建子弹增益
        pygame.time.set_timer(BUFF2_SHOW_UP, random.randint(20000, 40000))  # 创建炸弹增益
        pygame.time.set_timer(ENEMY_FIRE_EVENT, 2000) # 敌机发射子弹
